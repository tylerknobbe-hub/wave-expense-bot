...
export default function Home(){
  ...
  const runWave=useCallback(async()=>{if(!wTok||!bId){alert("Enter Wave token and Business ID in Settings first");return}setWStat("checking");setWErr("");try{console.log("Fetching Wave with biz:",bId);const ts2=await fetchWave(wTok,bId);console.log("Wave returned:",ts2.length,"transactions");setWEx(ts2);if(txns.length>0)setTxns(mergeWave(txns,ts2));setWStat("done")}catch(e){console.error("Wave error:",e);setWStat("error");setWErr(e.message||"Connection failed")}},[wTok,bId,txns]);

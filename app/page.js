...
// ─── WAVE & DEDUP ─────────────────────────────────
async function fetchWave(tok,biz){const q=`query($b:ID!,$p:Int!,$ps:Int!){business(id:$b){transactions(page:$p,pageSize:$ps,sortDirection:DESC){pageInfo{currentPage totalPages}edges{node{id date description amount{value}account{name}}}}}}`;const all=[];let p=1,tp=1;while(p<=tp&&p<=10){const r=await fetch("/api/wave",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token:tok,query:q,variables:{b:biz,p,ps:100}})});const d=await r.json();const td=d?.data?.business?.transactions;if(!td)break;tp=td.pageInfo.totalPages;for(const e of(td.edges||[])){const n=e.node;const dt=nd(n.date);if(dt>=TYS&&dt<=TYE)all.push({waveId:n.id,date:n.date,description:n.description||"",amount:Math.abs(parseFloat(n.amount?.value)||0),account:n.account?.name||"",hasReceipt:(n.description||"").includes("[receipt]")})}p++}return all}
...
function SB({source}){const m=SRC[source]||SRC.creditcard;return <Bd bg={`${m.color}18`} color={m.color}>{m.icon} {m.label}</Bd>}
  ...
  return (
    <div style={{background:T.card,borderRadius:14,border:`1px solid ${T.border}`,padding:24}}>
      ...
      <div style={{background:T.bg,borderRadius:10,padding:16,marginBottom:16}}>
        ...
        <div style={{fontSize:16,fontWeight:700,marginBottom:6}}>{t.rawMerchant || t.description}</div>
...
export default function Home(){
  ...
  const[imp,setImp]=useState(new Set());const[thresh,setThresh]=useState(THR_DEF);const[wStat,setWStat]=useState("idle");const[wEx,setWEx]=useState([]);
  ...
  const[triageItems,setTriageItems]=useState([]);const[triageExcluded,setTriageExcluded]=useState([]);const[showTriage,setShowTriage]=useState(false);
  ...
  const triageKeep=useCallback((t)=>{const tagged={...t,id:`triage-${Date.now()}-${Math.random().toString(36).slice(2)}`,status:t.confidence>=80?"approved":"pending",needsReceipt:t.amount>=thresh};setTxns(p=>[...p,tagged]);setTriageItems(p=>p.filter(x=>x!==t))},[thresh]);
  const triageDiscard=useCallback((t)=>{setTriageItems(p=>p.filter(x=>x!==t))},[]);
  ...
  const runWave=useCallback(async()=>{if(!wTok||!bId)return;setWStat("checking");try{const ts2=await fetchWave(wTok,bId);setWEx(ts2);if(txns.length>0)setTxns(mergeWave(txns,ts2));setWStat("done")}catch{setWStat("error")}},[wTok,bId,txns]);
  ...
  const grid="28px 56px 82px 1fr 148px 72px 40px 48px 60px";
      ...
      <main style={{maxWidth:1200,margin:"0 auto",padding:"24px 18px"}}>
        ...
        {page==="review"&&<><div style={{display:"flex",gap:7,marginBottom:16,flexWrap:"wrap"}}>{[["Total",C.all,T.text],["Approved",C.ok,T.green],["Pending",C.pend,T.amber],["Wave",C.wave,T.purple],["Dupes",C.dup,T.red],["No Receipt",C.norec,T.red]].map(([l,c,color])=><div key={l} style={{flex:1,minWidth:80,padding:"10px 12px",background:T.card,borderRadius:9,border:`1px solid ${T.border}`}}><div style={{fontSize:9,color:T.dim,textTransform:"uppercase"}}>{l}</div><div style={{fontSize:18,fontWeight:800,color}}>{c}</div></div>)}</div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}>
            <div style={{display:"flex",gap:3,flexWrap:"wrap"}}>{[["all","All"],["creditcard","💳"],["copilot","📊"],["doordash","🔴"],["ubereats","🟢"],["uber","🚗"],["wave","🌊"],["norec","📎!"],["large","💰"],["dup","🔄"]].map(([k,l])=><button key={k} onClick={()=>setFilt(k)} style={{...bt(),fontSize:10,padding:"4px 8px",background:filt===k?T.accentBg:"transparent",color:filt===k?T.accent:T.dim}}>{l}</button>)}</div>
        ...
        {page==="push"&&<div style={{maxWidth:580,margin:"20px auto"}}><h2 style={{fontSize:18,fontWeight:800,marginBottom:16,textAlign:"center"}}>🚀 Push — TY{TY}</h2>{!wTok||!bId?<div style={{textAlign:"center",padding:20,background:T.card,borderRadius:10,border:`1px solid ${T.border}`}}><button onClick={()=>setPage("settings")} style={{...bt(),background:T.accent,color:"#fff"}}>⚙ Configure Wave</button></div>:<>{wStat!=="done"&&<button onClick={runWave} style={{...bt(),width:"100%",marginBottom:12,background:T.purpleBg,color:T.purple,padding:10}}>🌊 {wStat==="checking"?"Checking...":"Check & Merge Wave"}</button>}<div style={{background:T.card,borderRadius:10,border:`1px solid ${T.border}`,padding:12,maxHeight:280,overflowY:"auto"}}>{txns.filter(t=>t.status==="approved").map(t=><div key={t.id} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:`1px solid ${T.border}`,fontSize:11}}><span style={{display:"flex",alignItems:"center",gap:5}}><SB source={t.source}/><span style={{maxWidth:230,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.description}</span></span><span style={{fontWeight:600}}>${t.amount.toFixed(2)}</span></div>)}</div><div style={{display:"flex",gap:7,marginTop:12}}><button onClick={()=>setPage("review")} style={{...bt(),flex:1}}>←</button><button style={{...bt(),flex:2,background:T.green,color:"#000",fontWeight:700}}>🚀 Push {C.ok} + Merge {C.wave}</button></div></>}</div>}

...
function mt(s){try{const h=new Date(s).getHours();const pacific=((h-8)+24)%24;return pacific<11?"breakfast":pacific<15?"lunch":"dinner"}catch{return"meal"}}
...
// ─── WAVE & DEDUP ─────────────────────────────────
// ─── RECEIPT UPLOAD TO WAVE ───────────────────
async function uploadReceiptToWave(apiToken, bizUrlId, imageBlob, filename) {
  const form = new FormData();
  form.append('file', imageBlob, filename || 'receipt.png');
  const r = await fetch('/api/wave-receipt', {
    method: 'POST',
    body: JSON.stringify({ token: apiToken, bizId: bizUrlId, filename }),
    headers: { 'Content-Type': 'application/json' }
  });
  // Actually we need multipart — let's use a dedicated route
  return null; // placeholder — handled in wave-receipt route
}
...
export default function Home(){
  ...
  const[rProg,setRProg]=useState(null);const[testPushResult,setTestPushResult]=useState(null);const[testPushing,setTestPushing]=useState(false);const[anchorAcct,setAnchorAcct]=useState("");const[expenseAcct,setExpenseAcct]=useState("");const[wSessionTok,setWSessionTok]=useState("");const[sel,setSel]=useState(new Set());const[sort,setSort]=useState({field:"date",dir:"asc"});
  ...
  const runWave=useCallback(async()=>{if(!wTok){alert("Enter Wave token in Settings first");return}setWStat("checking");setWErr("");try{
      // If no bId or bId fails, try to auto-detect business
      let useBId=bId;
      if(!useBId){
        console.log("No business ID, listing businesses...");
        const listQ=`query{businesses{edges{node{id name}}}}`;
        const lr=await fetch("/api/wave",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token:wTok,query:listQ,variables:{}})});
        const ld=await lr.json();
        if(ld?.errors)throw new Error(ld.errors[0]?.message||"API error");
        const bizList=ld?.data?.businesses?.edges?.map(e=>e.node)||[];
        if(bizList.length===0)throw new Error("No businesses found");
        useBId=bizList[0].id;
        setBId(useBId);
        console.log("Found businesses:",bizList.map(b=>`${b.name} (${b.id})`));
      }
      try{
        const{accounts,businessName}=await fetchWaveAccounts(wTok,useBId);
        setWAccts(accounts);setWBizName(businessName);setWStat("done");
      }catch(e2){
        // Business ID might be wrong format, try listing
        console.log("Direct fetch failed, listing businesses...");
        const listQ=`query{businesses{edges{node{id name}}}}`;
        const lr=await fetch("/api/wave",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token:wTok,query:listQ,variables:{}})});
        const ld=await lr.json();
        if(ld?.errors)throw new Error(ld.errors[0]?.message||"API error listing businesses");
        const bizList=ld?.data?.businesses?.edges?.map(e=>e.node)||[];
        if(bizList.length===0)throw new Error("No businesses found in your Wave account");
        // Use first business
        useBId=bizList[0].id;
        setBId(useBId);
        const{accounts,businessName}=await fetchWaveAccounts(wTok,useBId);
    ...
    }catch(e){console.error("Wave error:",e);setWStat("error");setWErr(e.message||"Connection failed")}},[wTok,bId]);

  const testPush=useCallback(async(txnId)=>{if(!wTok){alert("Configure Wave token first");return}if(!bId){alert("Click Check Wave first to detect your business");return}const t=txns.find(x=>x.id===txnId);if(!t){alert("Transaction not found");return}setTestPushing(true);setTestPushResult(null);try{const mutation=`mutation($input:MoneyTransactionCreateInput!){moneyTransactionCreate(input:$input){didSucceed inputErrors{path message code}transaction{id}}}`;const input={businessId:bId,externalId:`exp-${t.id}-${Date.now()}`,date:nd(t.date),description:(t.source==="ubereats"?`Uber Eats | ${t.restaurant||"order"}`:(t.source==="doordash"?`DoorDash | ${t.restaurant||"order"}`:(t.source==="uber"?`Uber Ride | ${t.pickup||"trip"}`:(t.rawMerchant||t.description||"Business expense").split(" — ")[0].split(" to discuss")[0].slice(0,100)))),notes:`${t.description||""}${t.receiptLink?" | Receipt: "+t.receiptLink:""}`,anchor:{accountId:anchorAcct||wAccts.find(a=>a.subtype?.value==="CREDIT_CARD")?.id||wAccts.find(a=>a.subtype?.value==="CASH_AND_BANK")?.id||wAccts[0]?.id||"",amount:t.amount,direction:"WITHDRAWAL"},lineItems:[{accountId:expenseAcct||wAccts.find(a=>a.type?.value==="EXPENSE")?.id||wAccts[0]?.id||"",amount:t.amount,balance:"INCREASE"}]};const r=await fetch("/api/wave",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token:wTok,query:mutation,variables:{input}})});const d=await r.json();if(d?.data?.moneyTransactionCreate?.didSucceed){const waveId=d.data.moneyTransactionCreate.transaction.id;setTestPushResult({success:true,id:waveId,desc:t.description});setTxns(p=>p.map(x=>x.id===txnId?{...x,status:"pushed",waveId}:x));
        // Upload receipt if we have one
        if(t.receiptFound&&t.receiptLink){try{const rr=await fetch("/api/wave-receipt",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token:wTok,bizUrlId:"ccdf6ab4-d13f-464e-a63d-e33d54b27aa8",receiptUrl:t.receiptLink,description:t.description,sessionToken:wSessionTok,waveTransactionId:waveId})});const rd=await rr.json();console.log("Receipt upload:",rd)}catch(re){console.error("Receipt upload failed:",re)}}
      }else{const errors=d?.data?.moneyTransactionCreate?.inputErrors||d?.errors||[];setTestPushResult({success:false,error:errors.map(e=>e.message||JSON.stringify(e)).join(", ")||"Unknown error"})}}catch(e){setTestPushResult({success:false,error:e.message})}setTestPushing(false)},[wTok,bId,txns]);
    ...
    <div style={{minHeight:"100vh",background:T.bg,color:T.text,fontFamily:"'Outfit','DM Sans',system-ui,sans-serif"}}>
      ...
      <header style={{background:T.card,borderBottom:`1px solid ${T.border}`,padding:"12px 22px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:50}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <span style={{fontSize:9,color:T.dim,background:T.accentBg,padding:"2px 6px",borderRadius:4}}>TY{TY}</span><span style={{fontSize:9,color:T.dim,background:`${T.purple}15`,padding:"2px 6px",borderRadius:4}}>{APP_VERSION}</span>
      ...
      <main style={{maxWidth:1200,margin:"0 auto",padding:"24px 18px"}}>
        ...
        {page==="settings"&&<div style={{maxWidth:500,margin:"28px auto"}}><h2 style={{fontSize:18,fontWeight:800,marginBottom:16}}>⚙ Settings</h2><div style={{background:T.card,borderRadius:14,border:`1px solid ${T.border}`,padding:22}}>
          ...
          <Sc t="Wave API"><In l="API Token" type="password" v={wTok} set={setWTok} ph="Wave Full Access token"/><In l="Business ID" v={bId} set={setBId} ph="Auto-detected if blank"/><In l="Session Token (for receipts)" type="password" v={wSessionTok} set={setWSessionTok} ph="From browser cookies — waveapps cookie value"/></Sc>

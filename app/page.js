...
export default function Home(){
  ...
  const[rProg,setRProg]=useState(null);const[testPushResult,setTestPushResult]=useState(null);const[testPushing,setTestPushing]=useState(false);const[sel,setSel]=useState(new Set());const[sort,setSort]=useState({field:"date",dir:"asc"});
  ...
  const testPush=useCallback(async(txnId)=>{if(!wTok||!bId){alert("Configure Wave API first");return}const t=txns.find(x=>x.id===txnId);if(!t){alert("Transaction not found");return}setTestPushing(true);setTestPushResult(null);try{const mutation=`mutation($input:TransactionCreateInput!){transactionCreate(input:$input){transaction{id description}didSucceed inputErrors{path message}}}`;const input={businessId:bId,externalAccountId:"",date:nd(t.date),description:t.description||"Business expense",notes:t.category||"",anchor:{accountId:"EXPENSE_ACCOUNT_ID",amount:t.amount,direction:"DEPOSIT"}};const r=await fetch("/api/wave",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token:wTok,query:mutation,variables:{input}})});const d=await r.json();if(d?.data?.transactionCreate?.didSucceed){setTestPushResult({success:true,id:d.data.transactionCreate.transaction.id,desc:t.description});setTxns(p=>p.map(x=>x.id===txnId?{...x,status:"pushed"}:x))}else{const errors=d?.data?.transactionCreate?.inputErrors||d?.errors||[];setTestPushResult({success:false,error:errors.map(e=>e.message||JSON.stringify(e)).join(", ")||"Unknown error"})}}catch(e){setTestPushResult({success:false,error:e.message})}setTestPushing(false)},[wTok,bId,txns]);
  ...
  return (
    <div style={{minHeight:"100vh",background:T.bg,color:T.text,fontFamily:"'Outfit','DM Sans',system-ui,sans-serif"}}>
      ...
      <main style={{maxWidth:1200,margin:"0 auto",padding:"24px 18px"}}>
        ...
        {page==="push"&&<div style={{maxWidth:580,margin:"20px auto"}}><h2 style={{fontSize:18,fontWeight:800,marginBottom:16,textAlign:"center"}}>🚀 Push — TY{TY}</h2>{!wTok||!bId?<div style={{textAlign:"center",padding:20,background:T.card,borderRadius:10,border:`1px solid ${T.border}`}}><button onClick={()=>setPage("settings")} style={{...bt(),background:T.accent,color:"#fff"}}>⚙ Configure Wave</button></div>:<>{wStat!=="done"&&<button onClick={runWave} style={{...bt(),width:"100%",marginBottom:12,background:T.purpleBg,color:T.purple,padding:10}}>🌊 {wStat==="checking"?"Checking...":"Check & Merge Wave"}</button>}
